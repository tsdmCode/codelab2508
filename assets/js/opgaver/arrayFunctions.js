...
// din kode her...
let myNextNumbers = myNumbers.map((element) => {
  element += 7;
...
let myLastNumbers = myNextNumbers.map((element) => {
  element *= 12;
...
const keldSucks = myNames.filter((navn) => {
  if (navn != 'keld') return navn;
});

console.log(keldSucks);
